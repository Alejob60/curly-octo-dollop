...
from app.services.phase_orchestrator import Phase, phase_guard
from app.services.persistence_bridge import persistence_bridge
from app.core.vertex_client import vertex_client
from app.utils.text_sanitizer import fix_tokenization, normalize_name, validate_and_clean_context
...
class PQRSManager:
    ...
    async def extract_basic_info(self, session_id: str, message: str) -> dict:
        ...
        # 1. Extracción Inteligente de Cédula (6-10 dígitos)
        doc_match = re.search(r'\b\d{6,10}\b', message)
        documento = doc_match.group(0) if doc_match else ""
        
        # 2. Extracción de Nombre (Regex mejorada)
        nombres = ""
        apellidos = ""
        name_patterns = [
            r"(?i)me llamo\s+([A-Za-záéíóúñÁÉÍÓÚÑ\s]+?)(?:,|\s+soy|\s+con)",
            r"(?i)soy\s+([A-Za-záéíóúñÁÉÍÓÚÑ\s]+?)(?:,|\s+con|\s+identificado)",
            r"(?i)representante:?\s+([A-Za-záéíóúñÁÉÍÓÚÑ\s]+?)(?:,|\s+JAC|\s+de)",
            r"(?i)peticionario:?\s+([A-Za-záéíóúñÁÉÍÓÚÑ\s]+?)(?:,|\s+cc|\s+c\.c)"
        ]
        for pattern in name_patterns:
            match = re.search(pattern, message)
            if match:
                full = match.group(1).strip().split()
                if len(full) >= 2:
                    nombres = full[0].title()
                    apellidos = " ".join(full[1:]).title()
                elif len(full) == 1:
                    nombres = full[0].title()
                break
        
        # 3. Municipio Multi-Tenant (Fallback a Cali)
        municipio = "Cali" 
        city_match = re.search(r"(?i)(?:de|en|municipio de)\s+(Cali|Medellín|Bogotá|Barranquilla|Cartagena)", message)
        if city_match:
            municipio = city_match.group(1).title()

        # 4. Construcción del Radicado (Soberanía Cali forzada si no es demo)
        dep_id = profile.get('DEPENDENCY_ID', 'GEN')
        radicado = f"CALI-{dep_id}-{session_id[-4:].upper()}"
        ...
        extraction_result = {
            ...
            "apellidos": apellidos,
            "municipio": municipio,
            "departamento": "Valle del Cauca" if municipio == "Cali" else "",
        ...
        logger.info(f"👁️ [GODS_EYE: EXTRACTION] {json.dumps(extraction_result, indent=2, ensure_ascii=False)}")
    ...
    async def background_process_full_analysis(self, session_id: str, message: str):
        ...
        try:
            logger.info(f"🧠 [DIAMOND_V64_STABLE] [BACKGROUND] Iniciando orquestación legal para {session_id}...")
            from app.services.legal_agents.orchestrator import legal_orchestrator
            from app.services.pdf_service import hard_sanitize_for_pdf
            ...
            # 3. Mapeo Inicial con Prioridad y Fallbacks (FIX CAPA 2)
            state_key = f"{self.state_prefix}{session_id}"
            
            # Obtener estado actual (lo que el usuario ya llenó en frontend)
            raw_state = await redis_client.hgetall(state_key)
            current_state = { (k.decode() if isinstance(k, bytes) else k): (v.decode() if isinstance(v, bytes) else v) for k, v in raw_state.items() }

            # --- IDENTIDAD: Prioridad Frontend ---
            final_nombres = (current_state.get("nombres") or getattr(agent_state, "nombres", "")).strip().title()
            final_apellidos = (current_state.get("apellidos") or getattr(agent_state, "apellidos", "")).strip().title()
            
            # Limpiar placeholders
            if "[NOMBRE" in final_nombres or len(final_nombres) < 2: final_nombres = "Peticionario"
            
            # --- HECHOS: Fallback Sustantivo ---
            hechos_raw = "\n".join(getattr(agent_state, "facts", [])) if getattr(agent_state, "facts", []) else ""
            if not hechos_raw or len(hechos_raw) < 30 or "no se extrajeron" in hechos_raw.lower():
                final_hechos = f"El ciudadano solicita el trámite basándose en el motivo: {hard_sanitize_for_pdf(message[:300])}"
            else:
                final_hechos = hard_sanitize_for_pdf(hechos_raw)

            # --- CITAS: Validación y Sanitización ---
            citas_raw = getattr(agent_state, "legal_basis", [])
            clean_citas = []
            for c in citas_raw:
                if isinstance(c, dict) and c.get("articulo") and c.get("texto_relevante"):
                    clean_citas.append({
                        "citacion_formato": hard_sanitize_for_pdf(c.get("citacion_formato", "Norma General")),
                        "articulo": c.get("articulo", "N/A"),
                        "texto_relevante": hard_sanitize_for_pdf(c.get("texto_relevante", "")),
                        "ente_emisor": c.get("ente_emisor", "Alcaldía de Cali")
                    })
            ...
            clean_data = {
                "documento": current_state.get("documento") or getattr(agent_state, "documento", ""),
                "nombres": final_nombres,
                "apellidos": final_apellidos,
                "borrador_proyeccion": hard_sanitize_for_pdf(agent_state.draft_document or ""),
                "hechos_extraidos": final_hechos,
                "citas_verificables": json.dumps(clean_citas),
                "motivo": message,
            ...
            # 5. Actualizar progreso para polling
    ...
    async def get_next_ui_instruction(self, session_id: str, data: dict = None) -> dict:
        ...
        if phase == Phase.F5_CONFIRMACION: 
            instruction = {"type": "command", "command": "READY_FOR_SIGNATURE", "cardType": "SuccessCard", "data": data}
            logger.info(f"👁️ [GODS_EYE: UI_INSTRUCTION] phase=F5_CONFIRMACION | instruction={json.dumps(instruction, indent=2, ensure_ascii=False)}")
            return instruction
            
        instruction = {"type": "card", "cardType": card_map.get(phase, "IdentityCard"), "data": data}
        logger.info(f"👁️ [GODS_EYE: UI_INSTRUCTION] phase={phase.value} | instruction={json.dumps(instruction, indent=2, ensure_ascii=False)}")
        return instruction

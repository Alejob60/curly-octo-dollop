...
class PersistenceBridge:
    ...
    async def save_progress(self, session_id: str, radicado: str, phase_data: Dict[str, Any]):
        """
        SPRINT 1.1: Upsert Atómico Resiliente. (God's Eye Trace)
        """
        logger.info(f"👁️ [GODS_EYE: DB_PERSISTENCE] session={session_id} | radicado={radicado} | data={json.dumps(phase_data, indent=2, ensure_ascii=False)}")
